# 🤖 AIRVIS (Jarvis × OpenClaw 통합 AI 비서)

> **더블 박수(👏 👏)로 깨어나 음성으로 소통하고 작업을 수행하는 데스크톱 AI 비서**

`hectorg2211/jarvis`의 박수 감지 및 ElevenLabs TTS 시스템을 기반으로, **OpenClaw AI Agent**와 **실시간 VAD 음성 인식(STT)** 파이프라인을 결합하여 완성한 개인 AI 비서입니다.
...
* **🧠 똑똑한 2-Track 명령 라우팅**:
  ...
  * **OpenClaw AI 위임**: 최신 뉴스, 날씨, 파일 정리, 복합 질의 등은 OpenClaw Agent가 처리.
  ...
  "Yes, sir. 무엇을 도와드릴까요?" (ElevenLabs TTS)
    ...
    음성 인식 (STT: Google / Whisper)
     ...
     [ 명령 라우팅 계층 (handle_command) ]
      /                                 \
  (로컬 제어 명령)                  (AI 질의 / 복합 작업)
    ↓                                   ↓
 Jarvis Desktop Automation          OpenClaw Agent
 (Chrome, Spotify, YouTube 등)     (웹 검색, 파일 작업 등)
      \                                 /
       \                               /
              ↓
          결과 텍스트 반환
...
#### 1) OpenClaw 설치 및 실행
OpenClaw가 로컬에 설치되어 있고 Gateway 서비스가 켜져 있어야 합니다.
...
# 2. 대화 모드 설정
# ==========================================
# true: 한 번 박수로 연속 대화 (종료/그만 할 때까지 유지)
# false: 1회 명령 실행 후 바로 대기 모드로 복귀
...
### 2. AI 에이전트 작업 (OpenClaw 처리)
* **"오늘 서울 날씨 어때?"** ➔ 실시간 날씨 정보 검색 후 음성 안내
* **"최신 주요 뉴스 알려줘"** ➔ 뉴스 요약 브리핑
* **"다운로드 폴더에서 오늘 받은 파일 찾아줘"** ➔ 파일 탐색 및 작업 수행
* **"파이썬으로 웹스크래핑하는 코드 간단히 설명해줘"** ➔ 코딩 질문 답변
...
| `JARVIS_VAD_SILENCE_SECONDS` | `0.6` | 말 끝난 후 무음 판정 시간(초) - 작을수록 빠른 응답 |
