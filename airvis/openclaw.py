"""First-class native AIRVIS OpenClaw runtime."""
...
class OpenClaw:
    """Native autonomous multi-agent runtime; no external OpenClaw process required."""

    name = "AIRVIS OpenClaw"
    version = "7.1.0"

    def __init__(self, workspace: Path | str | None = None, *, config: AirvisConfig | None = None,
                 options: OpenClawOptions | None = None, approval_handler: Any = None) -> None:
        ...
        self.engine = AirvisEngine(settings, workspace=root, approval_handler=approval_handler,
        ...
        if self.options.model:
            ...
            for agent in self.engine.agents.all():
                if agent.backend_id == "native":
                    agent.model = self.options.model
        self.workspace = root
    ...
    def cancel(self, workflow_id: str | None = None) -> bool:
        ...
        return bool(target and self.engine.cancel(target))

    async def events(self, workflow_id: str | None = None) -> list[dict[str, Any]]:
        target = workflow_id or self.session_id
        return self.engine.event_bus.history(workflow_id=target, limit=400) if target else []
