"""Human-facing entry point for the AIRVIS command line application."""
...
def _print_mapping(data: dict[str, Any], indent: int = 0) -> None:
    ...
    for key, value in data.items():
        ...
        if isinstance(value, dict):
            ...
            print(f"{prefix}{label}: {', '.join(_value(item) for item in value)}")
        else:
            print(f"{prefix}{label}: {_value(value)}")


def render(payload: Any, command: str) -> None:
    """Render structured CLI data for humans without changing engine contracts."""
    if command == "status" and isinstance(payload, dict):
        print("AIRVIS 8.2")
        print("────────────────────────────────────────")
        for key in ("version", "providers", "backends", "agents", "tools", "skills", "plugins"):
            if key in payload:
                value = payload[key]
                if isinstance(value, list):
                    ...
                    print(f"{key.title():<12} {_value(value)}")
        return
...
def main(argv: list[str] | None = None) -> int:
    ...
    # Explicit JSON is the stable machine interface. The legacy cli.main stays
    # untouched so existing integrations and tests continue to consume it.
    machine_json = "--json" in args
    if machine_json:
