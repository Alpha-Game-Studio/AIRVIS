...
The V4 environment variable names are preserved so existing ``.env`` files keep
...
#: id -> (base url, api-key env var, default model, local, quality, in-cost, out-cost)
OPENAI_COMPATIBLE: dict[str, dict[str, Any]] = {
    "openai": {
        ...
        "quality": 0.85, "cost_in": 0.15, "cost_out": 0.6,
    },
    "xai": {
        "base_url": "https://api.x.ai/v1", "key_env": "XAI_API_KEY",
        "model_env": "GROK_MODEL", "model": "grok-2-latest",
        "quality": 0.8, "cost_in": 2.0, "cost_out": 10.0,
    },
    "openrouter": {
        "base_url": "https://openrouter.ai/api/v1", "key_env": "OPENROUTER_API_KEY",
        "model_env": "OPENROUTER_MODEL", "model": "nousresearch/hermes-3-llama-3.1-405b",
        "quality": 0.78, "cost_in": 0.9, "cost_out": 0.9,
    },
    "ollama": {
        "base_url": "", "key_env": "", "model_env": "OLLAMA_MODEL", "model": "llama3.2",
        "local": True, "quality": 0.45, "cost_in": 0.0, "cost_out": 0.0,
    },
    "custom": {
        "base_url": "", "key_env": "AIRVIS_API_KEY", "model_env": "AIRVIS_MODEL", "model": "gpt-4o-mini",
        "quality": 0.6, "cost_in": 0.0, "cost_out": 0.0,
    },
}
...
def build_provider(provider_id: str, environ: dict[str, str] | None = None, *, timeout: float = 60.0) -> Provider:
    """Instantiate a single provider by id, reading credentials from ``environ``."""
    ...
    if token in LOCAL_ALIASES:
        ...
        return OpenAICompatibleProvider(
            ...
            f"{host}/v1",
            api_key="ollama",
            model=env.get(spec["model_env"], spec["model"]),
            timeout=timeout,
            local=True,
    ...
    if compatible is None:
        raise ConfigError(
            ...
            known=sorted({*OPENAI_COMPATIBLE, "anthropic", "gemini", "mock"}),
        )
    spec = compatible
    base_url = spec["base_url"] or env.get("AIRVIS_CUSTOM_BASE_URL", "http://127.0.0.1:8000/v1")
    headers = {}
    if token == "openrouter":
        headers = {"HTTP-Referer": "https://github.com/cuufi2fh-png/AIRVIS", "X-Title": "AIRVIS"}
    return OpenAICompatibleProvider(
        token,
        base_url,
        api_key=env.get(spec["key_env"], "") if spec["key_env"] else "",
        model=env.get(spec["model_env"], spec["model"]),
        timeout=timeout,
        local=bool(spec.get("local")),
        quality=spec["quality"],
        cost_per_million_input=spec["cost_in"],
        cost_per_million_output=spec["cost_out"],
...
def build_provider_registry(
    config: AirvisConfig | ProvidersConfig | None = None,
    *,
    environ: dict[str, str] | None = None,
    health: Any = None,
    event_bus: Any = None,
) -> ProviderRegistry:
    """Assemble the provider registry: mock is always present as a last resort."""
...
def provider_from_environment(environ: dict[str, str] | None = None) -> Provider:
    """V4-compatible helper: build the provider named by ``AIRVIS_PROVIDER``."""
    env = dict(os.environ if environ is None else environ)
    try:
        return build_provider(env.get("AIRVIS_PROVIDER", "mock"), env)
    except ConfigError:
        return MockProvider()
