...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--approve", action="store_true", help="auto-approve high-risk tool calls"
    )
    ...
    workflow.add_argument(
        "--approve", action="store_true", default=argparse.SUPPRESS,
        help="auto-approve high-risk tool calls",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    engine = AirvisEngine(
        ...
        approval_handler=always_approve if approve else None,
    )
    ...
    if handler is None:  # pragma: no cover - argparse already restricts this
...
def _health(engine: Any, args: Any) -> int:
    ...
    unhealthy = [
        name
        for section in ("providers", "backends")
        for name, status in report.get(section, {}).items()
        if isinstance(status, dict) and status.get("state") == "unhealthy"
    ]
...
def _providers(engine: Any, args: Any) -> int:
    if args.action == "test":
        ...
        try:
            result = run_blocking(
                engine.providers.generate(GenerationRequest(messages=[Message("user", "Reply with OK")]))
            )
        except Exception as exc:
...
def _backends(engine: Any, args: Any) -> int:
    if args.action == "health":
        ...
        _emit(run_blocking(engine.backends.health_check_all()))
        return 0
    _emit(engine.backends.list())
    return 0
...
def _agents(engine: Any, args: Any) -> int:
    if args.action == "show":
        if not args.target:
            ...
            return 2
        _emit(engine.agents.get(args.target).to_dict())
        return 0
    if args.action == "route":
        ...
        ranking = engine.router.rank(Task(description=args.target))
        _emit([item.to_dict() for item in ranking])
        return 0
    _emit(engine.agents.list())
    return 0
...
def _tools(engine: Any, args: Any) -> int:
    if args.action == "show":
        if not args.name:
            print("tool name is required", file=sys.stderr)
            return 2
        _emit(engine.tools.get(args.name).schema())
        return 0
    _emit(engine.tools.list())
    return 0
...
def _models(engine: Any, args: Any) -> int:
    ...
    _emit(ModelCatalog(engine.providers).list(local=True if args.local else None))
    return 0
...
def _plan(engine: Any, args: Any) -> int:
    ...
    plan = run_blocking(engine.planner.plan(args.request))
    _emit(plan.to_dict())
    return 0
...
def _workflow(engine: Any, args: Any) -> int:
    ...
    if args.action == "run":
        if not args.target:
            ...
            return 2
        result = run_blocking(engine.run(args.target, strategy=args.strategy))
        _emit(result.to_dict())
        return 0 if result.ok else 1
    if args.action == "status":
        if not args.target:
            print("a workflow id is required", file=sys.stderr)
            return 2
        _emit(engine.orchestrator.status(args.target))
        return 0
    if args.action == "cancel":
        if not args.target:
            print("a workflow id is required", file=sys.stderr)
            return 2
        _emit({"cancelled": engine.cancel(args.target)})
        return 0
    if args.action == "resume":
        if not args.target:
            print("a workflow id is required", file=sys.stderr)
            return 2
        result = run_blocking(engine.resume(args.target))
        _emit(result.to_dict())
        return 0 if result.ok else 1
    if args.action == "events":
        _emit(engine.store.list_events(args.target))
        return 0
    _emit(engine.store.list_workflows())
    return 0
...
def _task(engine: Any, args: Any) -> int:
    if args.action == "inspect":
        if not args.id:
            print("a task id is required", file=sys.stderr)
            return 2
...
def _tool(engine: Any, args: Any) -> int:
    ...
    try:
        ...
        print(f"AIRVIS tool error: invalid JSON arguments: {exc}", file=sys.stderr)
        return 2
    try:
        result = run_blocking(
            engine.tools.call(
                ...
                approval_handler=always_approve if args.confirm else None,
            )
        )
    except (AirvisError, ValueError, KeyError) as exc:
        print(f"AIRVIS tool error: {exc}", file=sys.stderr)
        return 2
    _emit(result.to_dict())
    return 0 if result.ok else 1
...
def _memory(engine: Any, args: Any) -> int:
    if args.action == "add":
        if not args.value:
            ...
            return 2
        _emit({"id": engine.memory.add(args.value)})
        return 0
    if args.action == "delete":
        if not args.value or not engine.memory.delete(args.value):
            print("memory id is required or was not found", file=sys.stderr)
            return 2
        print("deleted")
        return 0
    _emit(engine.memory.list())
    return 0
...
def _plugin(engine: Any, args: Any) -> int:
    ...
    if args.action == "create":
        print(manager.create(args.name))
    elif args.action == "remove":
        print("removed" if manager.remove(args.name) else "not found")
    else:
        print("updated" if manager.enable(args.name, args.action == "enable") else "not found")
    return 0
...
def _legacy_tasks(engine: Any, args: Any) -> int:
    ...
    if args.action == "list":
        _emit(runtime.task_list())
        return 0
    if not args.id or args.id not in runtime.tasks:
...
def _server_command(command: str) -> int:
    if command in {"start", "server"}:
        ...
        if not _wait_for_server():
            ...
            print("AIRVIS server failed to start", file=sys.stderr)
            return 1
    ...
    if command == "stop":
        ...
        try:
            ...
            print(f"AIRVIS stop error: {exc}", file=sys.stderr)
            return 1
        return 0
    # restart
    if PID_PATH.is_file():
    ...
    if not _wait_for_server():
        PID_PATH.unlink(missing_ok=True)
        print("AIRVIS server failed to restart", file=sys.stderr)
        return 1
